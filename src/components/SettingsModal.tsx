    ...
    <div className="fixed inset-0 z-50 overflow-hidden pointer-events-none">
      ...
      >
        ...
        <div className="flex justify-between items-center px-6 pt-5 pb-2 shrink-0">
          <span className="text-[11px] font-bold text-[#5f6368] uppercase tracking-widest">{userEmail || 'Google Account'}</span>
          <button 
            onClick={onClose} 
            className="p-1.5 hover:bg-[#e9e8e5]/60 rounded-full text-slate-500 hover:text-slate-800 transition-colors"
            title="Close"
          >
            <X size={18} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-4 custom-scrollbar">
          ...
          <div className="bg-white rounded-[24px] p-5 border border-[#e3e2e0] shadow-sm flex flex-col items-center text-center mb-4">
            ...
            <button 
              ...
              className="mt-4 px-5 py-2 bg-white border border-[#e3e2e0] hover:bg-[#e9e8e5]/40 rounded-full text-[#1f1f1f] text-[13px] font-bold transition-all shadow-xs active:scale-98"
            >
              Manage your Google Account
            </button>
          </div>

          {/* UTILITY INTEGRATIONS LIST */}
          <div className="space-y-1.5 pt-1">
            
            {/* 1. Public News Feed */}
            <button 
              onClick={() => { onResetToHome?.(); onClose(); }}
              className="w-full text-left px-4 py-3.5 hover:bg-[#e9e8e5]/40 rounded-2xl transition-all flex items-start gap-4 group"
            >
              <div className="shrink-0 pt-0.5">
                <Newspaper className="text-[#1a73e8] group-hover:scale-105 transition-transform" size={20} />
              </div>
              <div className="flex-1 min-w-0">
                <span className="text-[14px] font-bold text-[#1f1f1f] block leading-tight">Public News Feed</span>
                <span className="text-[11px] text-[#5f6368] block mt-1 leading-snug font-medium">Return to the original homepage</span>
              </div>
            </button>

            {/* 2. Follow via RSS Feed */}
            <div>
              <button 
                onClick={() => setShowRssPanel(!showRssPanel)}
                className={`w-full text-left px-4 py-3.5 rounded-2xl transition-all flex items-start gap-4 group ${showRssPanel ? 'bg-[#e9e8e5]/30' : 'hover:bg-[#e9e8e5]/40'}`}
              >
                <div className="shrink-0 pt-0.5">
                  <Rss className="text-[#f2a154] group-hover:scale-105 transition-transform" size={20} />
                </div>
                <div className="flex-1 min-w-0">
                  <span className="text-[14px] font-bold text-[#1f1f1f] block leading-tight">Follow via RSS Feed</span>
                  <span className="text-[11px] text-[#5f6368] block mt-1 leading-snug font-medium">Subscribe with feed reader apps</span>
                </div>
              </button>
              ...
              <AnimatePresence>
                {showRssPanel && (
                  ...
                  >
                    <div className="bg-white border border-[#e3e2e0] rounded-xl p-3 mt-1.5 flex flex-col gap-2 shadow-xs">
                      <span className="text-[10px] font-bold text-[#5f6368] uppercase tracking-wider">Feed XML URL</span>
                      <div className="flex items-center gap-1.5 bg-[#faf8f5] border border-[#e3e2e0] rounded-lg p-1.5">
                        <input 
                          type="text" 
                          readOnly 
                          value="https://noorpos.in/feed.xml" 
                          className="flex-1 bg-transparent text-[11px] text-[#1f1f1f] outline-none font-mono font-medium px-1 select-all"
                        />
                        <button 
                          onClick={() => {
                            navigator.clipboard.writeText("https://noorpos.in/feed.xml");
                            setCopiedRss(true);
                            setTimeout(() => setCopiedRss(false), 2000);
                          }}
                          className="p-1 bg-[#3c40c6] text-white hover:bg-[#3c40c6]/90 rounded-md transition-colors"
                          title="Copy Link"
                        >
                          {copiedRss ? <Check size={12} /> : <Copy size={12} />}
                        </button>
                      </div>
                    </div>
                  </motion.div>
            ...
            {/* 3. Settings & Preferences (Collapsible) */}
            <div className="pt-0.5">
              <button 
                onClick={() => setIsPreferencesOpen(!isPreferencesOpen)}
                className={`w-full text-left px-4 py-3.5 rounded-2xl transition-all flex items-start gap-4 group ${isPreferencesOpen ? 'bg-[#e9e8e5]/50' : 'hover:bg-[#e9e8e5]/40'}`}
              >
                <div className="shrink-0 pt-0.5">
                  <Settings className="text-[#ab47bc] group-hover:scale-105 transition-transform" size={20} />
                </div>
                <div className="flex-1 min-w-0">
                  <span className="text-[14px] font-bold text-[#1f1f1f] block leading-tight">Settings & Preferences</span>
                  <span className="text-[11px] text-[#5f6368] block mt-1 leading-snug font-medium">Manage alerts, notifications and cookie policies</span>
                </div>
                <div className="shrink-0 pt-1">
                  {isPreferencesOpen ? (
                    <ChevronUp size={16} className="text-[#5f6368]" />
                  ) : (
              ...
              <AnimatePresence>
                {isPreferencesOpen && (
                  <motion.div
                    ...
                    animate={{ height: 'auto', opacity: 1 }}
                    exit={{ height: 0, opacity: 0 }}
                    className="overflow-hidden bg-white border border-[#e3e2e0] rounded-2xl mt-2 p-4 space-y-4 shadow-xs"
                  >
                    {/* POWER INTEGRATIONS & TOOLS SECTION */}
                    <div className="space-y-2 pb-3 border-b border-slate-100">
                      <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-[#3c40c6]">Power Integrations & Tools</h4>
                    ...
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-[#3c40c6]">Alerts & Reminders</h4>
                    ...
                    <div className="space-y-3 pt-3 border-t border-slate-100">
                      <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-[#3c40c6]">Aesthetics & Themes</h4>
                      
                      {/* Theme Selection */}
                      <div className="space-y-1.5">
                        <span className="text-[9px] font-bold text-[#5f6368] block uppercase">Display Theme</span>
                        <div className="grid grid-cols-3 gap-1.5">
                          {(['light', 'dark', 'system'] as const).map((t) => (
                            <button
                              key={t}
                              onClick={() => setTheme(t)}
                              className={`py-1.5 rounded-lg border transition-all text-[10px] font-bold capitalize ${
                                theme === t 
                                  ? 'bg-[#3c40c6] text-white border-[#3c40c6]' 
                                  : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
                              }`}
                    ...
                    {/* VAULT SECURITY */}
                    <div className="space-y-3 pt-3 border-t border-slate-100">
                      <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-[#3c40c6]">Vault Security</h4>
                    ...
                    <div className="space-y-2 pt-3 border-t border-slate-100">
                      <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-[#3c40c6]">Guides & Legal</h4>
                    ...
                    {/* RECENTLY DELETED */}
                    <div className="space-y-2 pt-3 border-t border-slate-100">
                      <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-[#3c40c6]">Recently Deleted (15 Days Vault)</h4>
            ...
            {/* 4. Liked History */}
            <button 
              onClick={() => { onToggleLikedOnly?.(); onClose(); }}
              className={`w-full text-left px-4 py-3.5 hover:bg-[#e9e8e5]/40 rounded-2xl transition-all flex items-start gap-4 group ${isLikedOnly ? 'bg-pink-50 border border-pink-100/60' : ''}`}
            >
              <div className="shrink-0 pt-0.5">
                <Heart className="text-[#e91e63] group-hover:scale-105 transition-transform" size={20} fill={isLikedOnly ? "#e91e63" : "none"} />
              </div>
              <div className="flex-1 min-w-0">
                <span className="text-[14px] font-bold text-[#1f1f1f] block leading-tight">Liked History</span>
                <span className="text-[11px] text-[#5f6368] block mt-1 leading-snug font-medium">Previously liked history and bookmarks</span>
              </div>
            </button>

            {/* 5. Add Another Account */}
            <button 
              onClick={() => alert("This feature requires signing into another Google Account. Multiple account sessions are supported in production.")}
              className="w-full text-left px-4 py-3.5 hover:bg-[#e9e8e5]/40 rounded-2xl transition-all flex items-start gap-4 group"
            >
              <div className="shrink-0 pt-0.5">
                <UserPlus className="text-slate-500 group-hover:scale-105 transition-transform" size={20} />
              </div>
              <div className="flex-1 min-w-0">
                <span className="text-[14px] font-bold text-[#1f1f1f] block leading-tight">Add another account</span>
                <span className="text-[11px] text-[#5f6368] block mt-1 leading-snug font-medium">Link another email to switch on the fly</span>
              </div>
            </button>
